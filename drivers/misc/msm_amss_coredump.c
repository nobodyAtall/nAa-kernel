...
#define MPU_ENABLE 0xA84C0800
#elif defined(CONFIG_ARCH_MSM7X27)
#define MPU_ENABLE 0xA8240800
#endif
...
static struct proc_dir_entry *proc_amss_errlog;
static char *amss_errlog;
...
static ssize_t read_amss_errlog(struct file *file, char __user *buffer,
				size_t buflen, loff_t *fpos)
{
	loff_t pos = *fpos;
	ssize_t count, size;

	size = proc_amss_errlog->size;
	if (pos >= size)
		return 0;

	count = min(buflen, (size_t)(size - pos));
	if (copy_to_user(buffer, amss_errlog + pos, count))
		return -EFAULT;
...
static const struct file_operations proc_amss_errlog_operations = {
	.read		= read_amss_errlog,
};
...
static int create_misc_proc_entries(size_t size)
{
	proc_create("vibrator", S_IRUSR, NULL,
			&proc_vibrator_operations);
	if (amss_errlog) {
		proc_amss_errlog = proc_create("last_amsslog", S_IRUSR, NULL,
				&proc_amss_errlog_operations);
		if (!proc_amss_errlog)
			return -ENOMEM;

		proc_amss_errlog->size = size;
	}
...
{
	void *mpu_vaddr;
	unsigned long mpu_phys;

	mpu_phys = MPU_ENABLE;
	mpu_vaddr = ioremap(mpu_phys, PAGE_SIZE);
	if (mpu_vaddr == NULL) {
		printk(KERN_WARNING "Error in ioremaping %lx\n", mpu_phys);
		return -EFAULT;
	}
	writeb(0x0, mpu_vaddr);
	iounmap(mpu_vaddr);
	return 0;
}
...
static int kdump_amsscoredump_driver_probe(struct platform_device *pdev)
{
	...
	amss_errlog = smem_get_entry(SMEM_ERR_CRASH_LOG, &size);
	ret = create_misc_proc_entries(size);
	if (ret < 0) {
